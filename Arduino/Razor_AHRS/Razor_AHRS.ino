/***************************************************************************************************************
 * SPI support
 * This section added P W Becker Nov 2013.  See below for further notices.
 * 
 * Data is normally sent out from the Razor over the serial port, which is
 * not always convenient for embedded applications.  The Razor I2C hardware
 * is already in use, and software implmentations (on other pins) are not
 * fast enough.  However, most boards have an SPI interface which is often 
 * not used. E.g. the Leonardo does not share the SPI pins with other digital
 * pins.  This modification allows the Razor to be an SPI slave.
 *
 * HARDWARE NOTE: SS is pin 14 on the ATMega328P, and a wire will need to
 * be soldered to this pin.  Also, don't forget to level shift to 3.3V if
 * you are using a 5V board (a resistor bridge will not work at high speeds -
 * use an N-FET).  Note that if there are no other SPI slaves, then the SS
 * line (pin 14 on the ATMega328P) can be tied low.
 *
 * MASTER NOTES: Depending on the master hardware, you may need to call
 ...
 * is in angles mode.  If INTERACTIVE is not defined, then the data will
 * sent out as angles over SPI.
 *
 ***************************************************************************************************************/

/*********** USER SETUP AREA for SPI functionality ********************************/
// Comment this out to omit SPI functionality
#define SPI_SLAVE
// Comment this out for production deployment - no menu/serial output
//#define INTERACTIVE
...
#define ACCEL_Z_MIN ((float) -304)
#define ACCEL_Z_MAX ((float) 204)
...
#define GYRO_AVERAGE_OFFSET_X ((float) 27.6)
#define GYRO_AVERAGE_OFFSET_Y ((float) 15.6)
#define GYRO_AVERAGE_OFFSET_Z ((float) 11.2)
...
void Prepare_SPI()
{
  radians100[0] = yaw*100;
  radians100[1] = pitch*100;
  radians100[2] = roll*100;
#ifdef DEBUG__SPI
  Serial.print("Update required for YPR to SPI buf. SPI receive count ");
...
#else // not INTERACTIVE (must be SPI_SLAVE mode)
    Calc_YPR();
    Prepare_SPI();
#endif 
